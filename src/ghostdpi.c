...
#define GHOSTDPI_VERSION "v0.2.3rc3"
...
int main(int argc, char *argv[]) {
    ...
    printf(
        "GhostDPI " GHOSTDPI_VERSION
        ": Passive DPI blocker and Active DPI circumvention utility\n"
        "https://github.com/ItzJPPPMe/GhostDPI\n\n"
    );
    ...
    while ((opt = getopt_long(argc, argv, "123456789pqrsaf:e:mwk:n", long_options, NULL)) != -1) {
        switch (opt) {
            ...
            case 'x':
                ...
                break;
            default:
                puts("Usage: ghostdpi.exe [OPTION...]\n"
                " -p          block passive DPI\n"
                " -q          block QUIC/HTTP3\n"
    ...
    printf("Filter activated, GhostDPI is now running!\n");
